...
**qq_spider.py**： 此文件使用selenium，用来获取qq的cookie。最终保存到cookie_file 中。

**personal_information.py**： 用来获取每个好友的信息，保存在persionInfo 目录下，供后续的分析使用

**age_analy.py**：用来分析好友中年龄的分布，最终以柱状图的形式展示，保存在persionInfo目录下。依赖numpy，matplotlib。

**secanalysis.py** 分析好友中性别的分布，以饼图的形式展示，保存在persionInfo目录

**city_friend.py**：分析好友在城市的分布情况，以柱状图展示，保存在persionInfo目录

**get_image.py**：获取好友头像，并将所有的好友头像拼在一张图中，此脚本获取的所有图片保存在image 目录
...
此文件现需要在cmd中执行，
```shell
python get_moods_detail.py create_table  #用来创建数据库，并且导入数据
```

**get_single_report**：这个是个基于flask的Web程序，用于在浏览器中查看指定好友说说的简单报告。也需要单独执行，并且必须要在执行完 get_moods_details.py文件以生成moods.sqlite数据库文件，这个web程序才可以正确执行。  
此程序的运行，直接执行本文件夹中的index.py即可。  
需要先安装flask、pandas、sqlalchemy这3个库。  
执行index.py后，在浏览器中输入 http://localhost/qqnum=QQ号码 就可以查看到结果了
